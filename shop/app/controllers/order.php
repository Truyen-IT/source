...
class order extends DController{
    ...
    public function order_details($order_code)
    {
        ...
        $data['order_info']=$ordermodel->list_info($table_order_details,$cond_infor);
        $this->load->view('cpanel/header');
        $this->load->view('cpanel/menu');
        $this->load->view('cpanel/order/order_details',$data);
        $this->load->view('cpanel/footer');
    }
    public function order_comfirm($order_code)
    {
        $ordermodel=$this->load->model('ordermodel');
        $table_order='tbl_order';
        ...
        $result=$ordermodel->order_comfirm($table_order,$data,$cond);
        if($result==1){
