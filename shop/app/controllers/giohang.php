<?php
class giohang extends DController{//ten file trung voi ten class
    ...
    public function index(){
        return $this->giohang();//chay cai ben duoi
    }
    public function giohang(){//chay ra giao dien la cai nay ne
        Session::init();
    ...
    public function themgiohang(){
        Session::init();
      // Session::destroy();
        if(isset($_SESSION["shopping_cart"])){//truong hop trung
            $avaiable=0;
            ...
            if( $avaiable==0){//khong trung
                ...
                $_SESSION["shopping_cart"][]=$item;//mang =item
     ...
     public function dathang(){
        ...
        $odermodel=$this->load->model('ordermodel');
        $name=$_POST['name'];
        ...
        $result_order=$odermodel->insert_order($table_order,$data_order);
       ...
       if(Session::get("shopping_cart")==true){
        foreach(Session::get("shopping_cart") as $key => $value){
            ...
            $result_order_details=$odermodel->insert_order_details($table_order_details,$data_details);
       ...
       $mes['msg']="Đặt Hàng Thành Công";

       header("Location:".BASE_URL.'/giohang?msg='.urlencode(serialize($mes)));}
